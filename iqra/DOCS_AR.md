...
تدعم اللغة علامات الوقف العربية الشائعة:

- الفاصلة العربية `،` تُعامل كفاصلة بين العناصر أو كفاصل بين التعليمات عند السياق المناسب.
- الفاصلة المنقوطة العربية `؛` تُعامل كمنهي للتعليمة (مثل `;`).

مثال:

```iqra
عدد=١، ق=[١،٢]؛ اطبع طول(ق)؛
```
...
- `len(x)` / `طول(x)` — يعطي الطول لـ x عندما يكون نصًا أو قائمة أو قاموسًا.
